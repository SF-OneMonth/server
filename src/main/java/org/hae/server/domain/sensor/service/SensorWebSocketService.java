...
/**
 * WebSocket을 통한 실시간 데이터 브로드캐스트를 담당하는 서비스
 * Kafka Consumer로부터 수신한 센서 데이터와 알람 데이터를 클라이언트에게 브로드캐스트합니다.
 */
@Slf4j
@Service
...
public class SensorWebSocketService {
    ...
    /**
     * 실시간 센서 데이터를 WebSocket을 통해 브로드캐스트
     * 
     * @param sensorData 브로드캐스트할 센서 데이터
    ...
    public void sendRealtimeSensorData(SensorData sensorData) {
        try {
            ...
            log.debug("센서 데이터 WebSocket 브로드캐스트 완료: destination={}, data={}", destination, sensorData);
        } catch (Exception e) {
            log.error("WebSocket 브로드캐스트 실패: {}", sensorData, e);
            throw new WebSocketTransmissionException(e);
        }
    }

    /**
     * 알람 발생 정보를 WebSocket을 통해 브로드캐스트
     * 
    ...
    public void sendSensorAlarm(String equipmentCode, AlarmHistory alarmHistory) {
        try {
            ...
            log.debug("알람 데이터 WebSocket 브로드캐스트 완료: destination={}, alarm={}", destination, alarmHistory);
        } catch (Exception e) {
            log.error("알람 WebSocket 브로드캐스트 실패: equipmentCode={}, alarm={}", equipmentCode, alarmHistory, e);
            throw new WebSocketTransmissionException(e);
        }
    }
}